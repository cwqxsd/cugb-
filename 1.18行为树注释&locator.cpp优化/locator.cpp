...
void Locator::init(FieldDimensions fd,
...
{
    const double c = std::cos(pose_r2f.theta);
    const double s = std::sin(pose_r2f.theta);
    return FieldMarker{
        marker_r.type,
        c * marker_r.x - s * marker_r.y + pose_r2f.x,
        s * marker_r.x + c * marker_r.y + pose_r2f.y,
        marker_r.confidence
    };
}

// 快速版：cos/sin 由外部预计算（residual/finalAdjust tight loop 用）
static inline FieldMarker markerToFieldFrameFast(const FieldMarker& marker_r,
                                                const Pose2D& pose_r2f,
                                                double c, double s)
{
    return FieldMarker{
        marker_r.type,
...
{
    // 保留旧粒子
    const auto old_hypos = hypos;
    const int oldN = static_cast<int>(old_hypos.rows());

    int num = static_cast<int>(oldN * numShrinkRatio);
    if (num <= 0 || oldN <= 0) return 1;

    hypos.resize(num, 6);
    // hypos.setZero(); // 不必要：后续会覆盖写入

    // -----------------------------
    // 重采样（Systematic Resampling，O(N)）
    // 依赖 old_hypos.col(5) 为 CDF（单调递增，最后≈1）
    // -----------------------------
    const double invN = 1.0 / static_cast<double>(num);
    const double u0 = ((Eigen::ArrayXd::Random(1)(0) + 1.0) * 0.5) * invN; // [0, 1/N)

    int j = 0;
    for (int i = 0; i < num; ++i)
    {
        const double u = u0 + static_cast<double>(i) * invN;
        while (j < oldN - 1 && old_hypos(j, 5) < u) ++j;
...
{
    // 用平方距离比较，最后只 sqrt 一次；避免 pow()
    double minD2 = std::numeric_limits<double>::infinity();

    for (const auto& target : fieldMarkers)
    {
        if (target.type != marker.type) continue;

        const double dx = target.x - marker.x;
        const double dy = target.y - marker.y;
...
std::array<double, 2> Locator::getOffset(FieldMarker marker)
{
    double minD2 = std::numeric_limits<double>::infinity();
    const FieldMarker* nearestTarget = nullptr;
    ...
    if (!nearestTarget) return {0.0, 0.0};
    return {nearestTarget->x - marker.x, nearestTarget->y - marker.y};
}
...
double Locator::residual(const vector<FieldMarker>& markers_r, const Pose2D& pose)
{
    ...
    {
        const double dist = std::max(norm(marker_r.x, marker_r.y), 0.1);
        const auto marker_f = markerToFieldFrameFast(marker_r, pose, c, s);
        res += minDist(marker_f) / dist * 3.0; // 加权
    }
...
Pose2D Locator::finalAdjust(const vector<FieldMarker>& markers_r, const Pose2D& pose)
{
    ...
    {
        const auto marker_f = markerToFieldFrameFast(marker_r, pose, c, s);
        const auto offset = getOffset(marker_f);
        dx += offset[0];
        dy += offset[1];
    }
...
int Locator::calcProbs(const vector<FieldMarker>& markers_r)
{
    ...
    for (int i = 0; i < rows; i++)
    {
        ...
        const double res = residual(markers_r, pose);
        hypos(i, 3) = res;
    ...
    const double mean = hypos.col(3).mean();
    const double sqSum = ((hypos.col(3) - mean).square().sum());
    const double sigma = std::sqrt(sqSum / (rows - 1));
    const double mu = hypos.col(3).minCoeff() - muOffset * sigma;
    ...
    const double probSum = hypos.col(4).sum();
...
void Locator::logParticles()
{
    ...
    {
        const auto hypo = hypos.row(i);
        points.emplace_back(rerun::Position2D{
            static_cast<float>(hypo(0)),
            static_cast<float>(hypo(1))
        });
    }
...
NodeStatus SelfLocate1M::tick()
{
    ...
    if (marker.range > maxDist) {
        log->log(logPathF, rerun::TextLog(format("Failed, min marker Dist(%.2f) > maxDist(%.2f)", marker.range, maxDist)));
        return NodeStatus::SUCCESS;
    }

    if (!brain->isBoundingBoxInCenter(marker.boundingBox)) {
        log->log(logPathF, rerun::TextLog(format("Failed, boundingbox is not in the center area")));
        return NodeStatus::SUCCESS;
    }
    ...
    if (!allMarkers.empty()) {
        ...
        if (residual > brain->locator->residualTolerance) {
            log->log(logPathF, rerun::TextLog(format("Failed, validation residual(%.2f) > tolerance(%.2f)", residual, brain->locator->residualTolerance)));
            return NodeStatus::SUCCESS;
        }
    }
...
NodeStatus SelfLocate2X::tick()
{
    ...
    if (points.size() != 2) {
        log->log(logPathF, rerun::TextLog(format("Failed, point cnt(%d) != 2", static_cast<int>(points.size()))));
        return NodeStatus::SUCCESS;
    }
    ...
    if (p0.range > maxDist || p1.range > maxDist) {
        log->log(logPathF, rerun::TextLog(format("Failed, p0 range (%.2f) or p1 range (%.2f) > maxDist(%.2f)", p0.range, p1.range, maxDist)));
        return NodeStatus::SUCCESS;
    }

    double xDist = fabs(p0.posToField.x - p1.posToField.x);
    if (xDist > 0.5) {
        log->log(logPathF, rerun::TextLog(format("Failed, xDist(%.2f) > maxDist(%.2f)", xDist, 0.5)));
        return NodeStatus::SUCCESS;
    }
    ...
    if (fabs(yDist - mapYDist) > 0.5) {
        log->log(logPathF, rerun::TextLog(format("Failed, yDist(%.2f) too far (%.2f) from mapYDist(%.2f)", yDist, 0.5, mapYDist)));
        return NodeStatus::SUCCESS;
    }
    ...
    if (drift > maxDrift) {
        log->log(logPathF, rerun::TextLog(format("Failed, dirft(%.2f) > maxDrift(%.2f)", drift, maxDrift)));
        return NodeStatus::SUCCESS;
    }
...
NodeStatus SelfLocateBorder::tick()
{
    ...
    if (!touchLineFound && !goalLineFound && !middleLineFound) {
        log->log(logPathF, rerun::TextLog("No touchline or goalline or middleLine found."));
        return NodeStatus::SUCCESS;
    }
